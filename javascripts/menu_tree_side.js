...
var menu_tree_side = function(selector, w, h) {
  ...
  this.svg = d3.select(selector).append('svg:svg')
	...
	var outer_bbox = d3.select('#outer_canvas').node().getBoundingClientRect();
...
menu_tree_side.prototype.update = function(json) {
  ...
  this.eltEnter.append('svg:circle')
    ...
    .style('font-family','Source Sans Pro')
		.style('font-size', function(d) { return d.fontsize ? d.fontsize : Math.min(0.5* radius(d)*0.925,100)/2 + 'px'; })
...
menu_tree_side.prototype.flatten = function(root) {
  var nodes = [], i = 0;
...
menu_tree_side.prototype.click = function(d) {
	...
	if( d.divid == "home"  ) {
		...
		d3.select('#profilecodeflower').style('display','none');
		...
		d3.select('#profilecodeflower').style('display','block');
		d3.select('#profilecodeflower').style('width','10vw');
		d3.select('#profilecodeflower').style('height','70vh');
...
menu_tree_side.prototype.mouseover = function(d) {

};

menu_tree_side.prototype.mouseout = function(d) {
 
};

menu_tree_side.prototype.tick = function() {
  var h = this.h;
  var w = this.w;
...
menu_tree_side.prototype.cleanup = function() {
  this.update([]);
  this.force.stop();
};
